...
describe('Ableron Express Middleware', () => {
  ...
  it('should pass request headers to resolveIncludes()', async () => {
    ...
    const server = express()
      .use(ableron())
      .get('/', (req: Request, res: Response) => {
        res.status(200).send(`<ableron-include src="${getFragmentBaseUrl(req)}/fragment" headers="X-Test">fallback</ableron-include>`);
      })
      .get('/fragment', (req: Request, res: Response) => {
        res.status(200).send(req.headers['x-test']);
      });

    // when
    const response = await request(server).get('/').set('X-TEST', 'test');
